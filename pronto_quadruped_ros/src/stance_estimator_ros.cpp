     ...
     StanceEstimator(feet_forces), nh_(nh)
{
    ...
    if(!nh_->get_parameter(legodo_prefix + "stance_mode", stance_mode)){
      ...
      RCLCPP_WARN(nh->get_logger(),"Invalid stance mode from param server. Using threshold with default 50 N.");
      setMode(Mode::THRESHOLD);
    }

    switch(mode_){
    case Mode::THRESHOLD:
      if(!nh_->get_parameter(legodo_prefix + "stance_threshold", stance_threshold)){
        RCLCPP_WARN(nh->get_logger(),"Could not read the stance threshold from param server. Using default 50 N.");
      }
      break;
    case Mode::HYSTERESIS:
      if(!nh_->get_parameter(legodo_prefix + "stance_hysteresis_low", hysteresis_low)){
        RCLCPP_WARN(nh->get_logger(),"Could not read the stance_hysteresis_low from param server. Using default 50 N.");
      }
      if(!nh_->get_parameter(legodo_prefix + "stance_hysteresis_high", hysteresis_high)){
        RCLCPP_WARN(nh->get_logger(),"Could not read the stance_hysteresis_high from param server. Using default 50 N.");
      }
      if(!nh_->get_parameter(legodo_prefix + "stance_hysteresis_delay_low", stance_hysteresis_delay_low_int)){
        RCLCPP_WARN(nh->get_logger(),"Could not read the stance_hysteresis_delay_low from param server. Using default 0 ns.");
      }
      if(!nh_->get_parameter(legodo_prefix + "stance_hysteresis_delay_high", stance_hysteresis_delay_high_int)){
        RCLCPP_WARN(nh->get_logger(),"Could not read the stance_hysteresis_delay_high from param server. Using default 0 ns.");
      }
    ...
    case Mode::REGRESSION:
      if(!nh_->get_parameter(legodo_prefix + "stance_regression_beta", beta)){
        RCLCPP_WARN(nh->get_logger(),"Could not read the stance_regression_beta from param server. Setting mode to THRESHOLD with default value of 50 N.");
        setMode(Mode::THRESHOLD);
      }
