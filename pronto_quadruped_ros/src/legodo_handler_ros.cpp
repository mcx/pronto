...
LegodoHandlerBase::Update *LegodoHandlerBase::computeVelocity()
{
    if (debug_) {
        ...
        prior_accel_msg.header.stamp = fromNSec(nsec_);
        ...
        sensor_msgs::msg::JointState prior_joint_accel_msg;
        ...
        prior_vel_msg.header.stamp = fromNSec(nsec_);
    ...
    if (leg_odometer_->estimateVelocity(utime_,
        ...
        if (debug_) {
            ...
            vel_sigma_bound_msg_.header.stamp = fromNSec(nsec_);

            vectToMsg(r_legodo_, vel_sigma_bound_msg_.plus_one_sigma);
            vectToMsg(xd_ + r_legodo_, vel_sigma_bound_msg_.velocity_plus_one_sigma);
        ...
        return new pronto::RBISIndexedMeasurement(RigidBodyState::velocityInds(),
                                                 xd_,
                                                 cov_legodo_,
                                                 LegodoHandlerBase::Update::legodo,
                                                 utime_);
    } else {
        if (debug_) {
