...
void SchmittTrigger::updateState(uint64_t present_time, double value) {
  ...
  if(verbose){
    std::cerr << "ST: " << value << " N , timer: " << timer << " ns, status is: " << current_status << std::endl;
  }

  // The timing logic should be rewritten with ExpireTimer at the next opportune moment
  if (current_status) {
    if (value <= falling_edge_threshold){
      if(verbose) std::cerr << "below threshold\n";
      if (timer >= falling_edge_time_delay) {
        if (verbose) std::cerr << "high state -> low trigger\n";
        current_status = false;
      } else {
        if (verbose) std::cerr << "high state but clock rising for low\n";
        timer += (present_time - previous_time);
        if (verbose) std::cerr << "timer is "<<timer<< " and the time are prev/act"<<previous_time<<"/"<<present_time<<std::endl;
      }
    } else {
      if (verbose) std::cerr << "high state, clock zero\n";
      timer = 0;
    }
  } else {
    if (value >= rising_edge_threshold) {
      if(verbose){
        std::cerr << "above threshold\n";
        std::cerr << "TIMER: " << timer << std::endl;
        std::cerr << "NOW - previous" << (present_time - previous_time) << std::endl;
      }
      if (timer >= rising_edge_time_delay) {
        if (verbose) std::cerr << "low state -> high trigger "<< timer<< "\n";
        current_status = true;
      } else {
        if (verbose) std::cerr << "low state but clock rising for high\n";
        timer += (present_time - previous_time);
      }
    } else {
      if (verbose) std::cerr << "low state, clock zero\n";
      timer = 0;
    }
  }
  previous_time = present_time;
